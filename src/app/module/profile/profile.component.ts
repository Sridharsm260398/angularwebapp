...
export class ProfileComponent implements OnInit {
  ...
  openedTabs:any= [];
 activeTab:any = null;
 showContent(content: string, label: string) {
   const existingTab= this.openedTabs.find((tab: { content: string; }) => tab.content === content);
   if (existingTab as any) {
     this.activeTab = existingTab;
   } else {
     const newTab:any = { content, label };
 ...
 closeTab(index: number) {
  ...
  toggleEdit() {
    //if (this.selectedContent == 'addresses' || this.activeTab == 'addresses') {
  ...
  toggleEditPAN(){
   // this.isEditingPAN = true;
    this.isEditingPAN = !this.isEditingPAN;
  }
  editProfile() {}
